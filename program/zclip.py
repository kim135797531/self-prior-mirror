...
# -*- coding: utf-8 -*-
"""zclip.py
Created by Dongmin Kim at 25. 8. 26.

ZClip: Adaptive Spike Mitigation for LLM Pre-Training
https://github.com/bluorion-com/ZClip/tree/main
...
class ZClip:
    ...
    def state_dict(self):
        """Mutable state for checkpoint (buffer, initialized, mean, var)."""
        return {
    ...
    def _compute_grad_norm(self, model):
        ...
        if is_fsdp_model(model):
            local_norm_sq = torch.stack(
                [
                    p.grad.to(dtype, non_blocking=True).norm(2).pow(2)
            ...
            grad_norms = [
                p.grad.to(dtype, non_blocking=True).norm(2)
                for p in model.parameters()
                if p.grad is not None
            ]
            if not grad_norms:
                return 0.0
            grad_norms_tensor = torch.stack(grad_norms).to(device, non_blocking=True)
    ...
    def step_with_parameters(self, parameters: iter):
        """
        (New method by d-kim, using parameters directly)
        """
        # 1. Convert to a list to avoid exhausting the generator.
        params_list = list(parameters)

        # Guard against empty parameter collections.
        if not params_list:
            return 0.0

        # 2. Safely access the first element via list indexing.
        first_param = params_list[0]
        device = first_param.device
        dtype = first_param.dtype

        # 3. Iterate over params_list to avoid missing any parameter.
        grad_norms = [
            p.grad.to(dtype, non_blocking=True).norm(2)
        ...
        if not grad_norms:
            ...
            grad_norms_tensor = torch.stack(grad_norms).to(device, non_blocking=True)
            total_norm = torch.sqrt(torch.sum(torch.pow(grad_norms_tensor, 2))).item()
        ...
        if not self.initialized:
            ...
            if len(self.buffer) >= self.warmup_steps:
                self._initialize_ema()
            if self.max_grad_norm is not None:
                torch.nn.utils.clip_grad_norm_(params_list, self.max_grad_norm)
            return total_norm
        ...
        # Calculate the clipping coefficient.
        clip_coef = (
            (effective_clip / (total_norm + 1e-6))
